#!/usr/bin/env bash
# Weryfikacja instalacji patcha — Linux / SteamOS.
set -euo pipefail
...
echo "=== West of Loathing — weryfikacja patcha (Linux) ==="
echo ""

if [[ ! -f "$PATCHES/core" ]]; then
  echo "BLAD: brak patches/core — uruchom: python scripts/pack.py"
  exit 1
fi
...
FOUND=0
if GAME_STREAMING="$(resolve_game_streaming "$ROOT" "${LINUX_CANDIDATES[@]}" 2>/dev/null)"; then
...
echo "Menu graficzne: python scripts/verify-ui-patch.py"
